    ...
    self.lib.docs.function
      {
        description = ''
          ...
          You pick which config field you want to expose (via `config`), and this function returns only the modules
          that provide it, plus a sensible `default` module. This is useful for turning a large flake module collection
          ...
          { lib, ... }:
          {
            options = {
              flakeModules = lib.mkOption {
                ...
                description = "Candidate flake modules to turn into submodules (keyed by name).";
              };

              specialArgs = lib.mkOption {
                type = lib.types.attrs;
                description = "Extra args used during evaluation (like `specialArgs` in `lib.evalModules`).";
              };

              config = lib.mkOption {
                type = lib.types.str;
                description = "Which config field to extract from each module (e.g. `nixosConfigurations`, `packages`, etc.).";
              };

              defaultConfig = lib.mkOption {
                type = lib.types.str;
                description = "Config flag name used to choose the default module; if none is marked, a default is generated.";
              };
            };
          }
