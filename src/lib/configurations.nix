    ...
    self.lib.docs.function
      {
        description = ''
          ...
          For each module that provides `''${config}`, this evaluates a `lib.nixosSystem` using the module’s `''${nixpkgsConfig}`
          (or Perch’s default systems) and returns an attrset of configurations keyed like:
          ...
          This is useful when you want a module-driven way to generate `nixosConfigurations` (including per-system defaults)
          ...
          { lib, ... }:
          {
            options = {
              specialArgs = lib.mkOption {
                ...
                description = "Extra args passed through to `lib.nixosSystem` and module evaluation (like `specialArgs`).";
              };

              flakeModules = lib.mkOption {
                type = lib.types.attrsOf lib.types.deferredModule;
                description = "Attrset of flake modules to turn into NixOS configurations (keyed by module name).";
              };

              nixpkgs = lib.mkOption {
                type = lib.types.path;
                description = "Path to a nixpkgs input used indirectly via `lib.nixosSystem` (for system-specific evaluation).";
              };
              ...
              defaultConfig = lib.mkOption {
                type = lib.types.str;
                description = "Config flag name that marks a configuration as the default for its system (emits `default-<system>`).";
              };
            };
          }
        )) lib.types.attrs;
