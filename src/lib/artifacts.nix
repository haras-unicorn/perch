    ...
    self.lib.docs.function
      {
        description = ''
          ...
          This evaluates each module for one or more target systems (based on its `''${nixpkgsConfig}` settings, or a default set),
          then extracts the requested `''${config}` value into an output shaped like:

          - `result.<system>.<module> = <value>`
          - and, when a module marks itself as default via `''${defaultConfig}`, also:
            `result.<system>.default = <value>`

          This is useful for producing flake outputs like per-system packages/apps/checks from a shared module collection,
          ...
          { lib, ... }:
          {
            options = {
              specialArgs = lib.mkOption {
                ...
                description = "Extra args used during module evaluation (passed through like `specialArgs`).";
              };

              flakeModules = lib.mkOption {
                type = lib.types.attrsOf lib.types.deferredModule;
                description = "Attrset of flake modules to evaluate and extract artifacts from (keyed by module name).";
              };

              nixpkgs = lib.mkOption {
                type = lib.types.path;
                description = "Path to a nixpkgs input, used to instantiate `pkgs` for each target system.";
              };
              ...
              config = lib.mkOption {
                type = lib.types.str;
                description = "Name of the config field to extract as the artifact value (supports top-level or `config.<name>`).";
              };

              defaultConfig = lib.mkOption {
                type = lib.types.str;
                description = "Config flag name that marks an artifact as the per-system default.";
              };
            };
          }
        )) lib.types.attrs;
