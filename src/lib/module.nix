...
in
{
  flake.lib.module.patch =
    self.lib.docs.function
      {
        description = "Patch a module (or module path) by rewriting its function args declaration/values and mapping its resulting attrset, recursively applying the same patch to any imported modules.";
