...

{
  flake.lib.eval.preEval =
    self.lib.docs.function
      {
        description = ''
          Evaluate a set of modules with an extra “eval module” prepended, after first patching each module so it only receives the args it explicitly requests.
          This is mainly used as a safe pre-pass for flake module evaluation where we want to avoid accidental arg leakage.
        '';
  ...
  flake.lib.eval.filter =
    self.lib.docs.function
      {
        description = ''
          Given an attrset of modules, evaluate each one in isolation (capturing its produced config/options),
          then run a predicate (filterModule) to decide which modules to keep.
          Returns a new attrset containing only the modules that passed the predicate.
        '';
        type = self.lib.types.function lib.types.attrs (
    ...
    self.lib.docs.function
      {
        description = ''
          ...
          Stage 2: re-evaluate with arg filtering and config path filtering applied, and produce `flake.modules`
          suitable for consumption by other flakes (including a generated `default` module).
        '';
        type = self.lib.types.function lib.types.attrs (
